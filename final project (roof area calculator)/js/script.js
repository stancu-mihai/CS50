...
function CreateFirstRow(no, newshape, selectedShape)
  {
    ...
    labelname.textContent = 'Shape'+no;
...
function CreateGroupForTrapeze(no)
  {
	...
	CreateRow('slope' + no , 'Slope (deg)' , newshape, false, false, no);
	...
	CreateTypeRowWithLabel(no, 'Left side', 'typeleft'+no, newshape);
	CreateTypeRowWithLabel(no, 'Right side', 'typeright'+no, newshape);
    CreateRow('hip' + no,    'Hip(mm)'   , newshape, false, true, no);
	CreateRow('valley' + no,  'Valley(mm)'   , newshape, false, true, no);
	CreateRow('rake' + no,   'Rake(mm)'       , newshape, false, true, no);
