...
char* indexes(const char* path)
{
    char* path1;
    char* path2;
    // Add index.php to path
    path1 = malloc(strlen(path) + 9 + 1);
    strcpy(path1, path);
    path1 = strcat(path1, "index.php");
    // Add index.html to path
    path2 = malloc(strlen(path) + 10 + 1);
    strcpy(path2, path);
    path2 = strcat(path2, "index.html");
    // Declare files
...
bool load(FILE* file, BYTE** content, size_t* length)
{
    ...
    // Store the first bytes of buffer in content
    (*content) = buffer;
    (*length) = size;
    if (buffer)
    {
...
const char* lookup(const char* path)
{
    // Search path for last '.' character
    char* pChar = strrchr(path, '.');
    if (strcasecmp(pChar, ".css") == 0)
    {
        return "text/css";
    }
    else if (strcasecmp(pChar, ".html") == 0)
    {
        return "text/html";
    }
    else if (strcasecmp(pChar, ".gif") == 0)
    {
        return "image/gif";
    }
    else if (strcasecmp(pChar, ".ico") == 0)
    {
        return "image/x-icon";
    }
    else if (strcasecmp(pChar, ".jpg") == 0)
    {
        return "image/jpeg";
    }
    else if (strcasecmp(pChar, ".js") == 0)
    {
        return "text/javascript";
    }
    else if (strcasecmp(pChar, ".php") == 0)
    {
        return "text/x-php";
    }
    else if (strcasecmp(pChar, ".png") == 0)
    {
        return "image/png";
    }
    else 
...
bool parse(const char* line, char* abs_path, char* query)
{
    ...
    // Get position of last space character
    char* secondSP = strrchr(line, ' ');
    // Get position of \r\n
    char* lastChar = strstr(line, "\r\n");
    // Get position of question mark, if any
    char* question_pos = strchr(line, '?');
    
    // Allocate space for method, length between line and method
    method = (char*) calloc(firstSP - line + 1, 1);
    // Copy memory to method, from start of line until the first space char
    memcpy(method, line, firstSP - line);
    // Finish the string
    method[firstSP - line] = '\0';
    
    // Allocate space for request_target, length between firstSP and secondSP
    request_target = (char*) calloc(secondSP - firstSP + 1,1);
    // Copy memory to request_target, from firstSP until the secondSP
    memcpy(request_target, firstSP + 1, secondSP - firstSP - 1);
    // Finish the string
    request_target[secondSP - firstSP] = '\0';
    
    // Allocate space for HTTP_version, length secondSP and lastChar
    HTTP_version = (char*) calloc(lastChar - secondSP + 1,1);
    // Copy memory to HTTP_version, from secondSP to lastChar
    memcpy(HTTP_version, secondSP + 1, lastChar - secondSP - 1);
    // Finish the string
    HTTP_version[lastChar - secondSP] = '\0';

    // If the request target contains ?
    if (question_pos != NULL)
    {
        // Allocate space for abs_path2, length question_pos - firstSP
        abs_path2 = (char*) calloc(question_pos - firstSP + 1,1);
        // Copy memory to abs_path, from firstSP to question mark pos
        memcpy(abs_path2, firstSP + 1, question_pos - firstSP - 1);
        // Finish the string
        abs_path2[question_pos - firstSP] = '\0';
        strcpy(abs_path, abs_path2);
    
        // Allocate space for query, length secondSP - question pos
        query2 = (char*) calloc(secondSP - question_pos + 1,1);
        // Copy memory to query, from question mark + 1 to secondPS
        memcpy(query2, question_pos + 1, secondSP - question_pos - 1);
        // Finish the string
        query2[secondSP - question_pos] = '\0';
        strcpy(query, query2);
    }
    else
    {
    ...
    // If the first character of request target is not /
    ...
    // If HTTP version is different from 1.1
    else if (strncmp(HTTP_version,"HTTP/1.1", 8))
    {
        error(505);
        free(method);
        free(request_target);
